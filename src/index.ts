...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'fyyd');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    description: 'fyyd, the open podcast directory — search podcast shows by title term, optionally restricted to a language code, up to 30 per page. Returns each show\'s fyyd podcast id, title, author, description, RSS feed URL, artwork and episode count. Answers which podcast shows exist under a name and where the RSS feed is.',
    inputSchema: {
      type: 'object',
      properties: {
        term: { type: 'string' },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'top_podcasts': {
      ...
      if (args.lang) params.set('language', String(args.lang));
      // fyyd moved the hot list under /feature. `/podcast/hot` 404s for every
      // caller on every call — this tool has served 7 requests and zero
      // successes in the last 7 days, and being a list endpoint with no
      // required argument, there was never a caller mistake that could explain
      // it. Verified: old path 404, `/feature/podcast/hot?count=2` returns 200
      // with the same `{status, msg, data:[…]}` envelope, and `category` is
      // still honoured there.
      return fyydGet(`/feature/podcast/hot?${params}`);
    }
    case 'categories':
